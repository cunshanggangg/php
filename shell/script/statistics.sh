...
function countAll() {
	git log --author=cunshanggangg --shortstat --pretty=format:"" | sed /^$/d >.tmp.count
	count;
	rm .tmp.count
}
...
function countToday() {
	...
	git log --author=cunshanggangg --since="$minutes seconds ago" --shortstat --pretty=format:"" | sed /^$/d >.tmp.count
	count;
	rm .tmp.count
...
function countOneDay() {
	git log --author=cunshanggangg --since="1 days ago" --shortstat --pretty=format:"" | sed /^$/d >.tmp.count
	count;
	rm .tmp.count
